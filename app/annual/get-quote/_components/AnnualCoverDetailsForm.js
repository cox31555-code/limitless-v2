...
const INSURANCE_LEVELS = [
  ...
  },
  {
    ...
    description: "Covers claims made by a third party in the event of an accident caused by yourself or any named drivers. There is no cover for you or your car."
  }
];
...
const AnnualCoverDetailsForm = ({ form }) => {
  ...
  const minimumCoverLevel = form.watch("coverDetails.minimumCoverLevel");
  const startDate = form.watch("coverDetails.startDate");

  const handleLevelChange = (levelId) => {
    form.setValue("coverDetails.level", levelId, { shouldValidate: true });
    // Reset minimumCoverLevel when changing cover type
    if (levelId !== "comprehensive") {
      form.setValue("coverDetails.minimumCoverLevel", "", { shouldValidate: true });
    }
  };

  const handleMinimumCoverChange = (e) => {
    form.setValue("coverDetails.minimumCoverLevel", e.target.value, { shouldValidate: true });
  };
