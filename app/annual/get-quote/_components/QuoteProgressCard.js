...
const QuoteProgressCard = ({ currentStep, vehicleSubStep, personalSubStep, coverSubStep, onSubStepClick, insuranceType }) => {
  ...
  const [isLargeScreen, setIsLargeScreen] = useState(typeof window !== 'undefined' && window.innerWidth > 900);
  ...
  const steps = [
    ...
    },
    {
      number: STEPS.COVER,
      title: "Your policy",
      // For temporary/impound, exclude "Additional drivers" and "Additional products"
      subSteps: isTemporaryOrImpound
        ? ["Car owner", "Cover details", "No claims discount", "Contact information"]
  ...
  useEffect(() => {
    if (stepsListRef.current) {
      ...
      if (activeSubStepElement) {
        ...
        // Check if this is the first substep (index 0)
        const activeIndex = getActiveSubStepIndex(currentStep);
        const extraPadding = activeIndex === 0 ? 12 : 10; // 12px for first substep, 10px for others
