"use client";
import React, { useState, Suspense } from "react";
import dynamic from "next/dynamic";
import Breadcrumb from "@/ui/dashboard/breadcrumb/Breadcrumb";
import styles from "./quotesPageClient.module.css";
import QuoteCard from "./quoteCard/QuoteCard";

const NeedHelpSection = dynamic(() => import("@/ui/layout/NeedHelpSection"), {
  loading: () => <div style={{ padding: "20px", textAlign: "center", color: "#999" }}>Loading...</div>,
  ssr: false,
});

const QuotesPageClient = () => {
  const [activeTab, setActiveTab] = useState("active");

  const quotes = [
    {
      id: 1,
      policyType: "PAY PER MILE",
      title: "Pay Per Mile",
      description: "Requires self-installed mileage tracker for mileage tracking. Flat-rate option available.",
      policyholderName: "John Smith",
      vehicleReg: "LN60 AYT",
      vehicleMake: "Ford",
      vehicleModel: "Fiesta",
      createdDate: "15 Jan 2024",
      expiresInDays: 28,
      pricePerMonth: "13.50",
      times: "11x",
      deposit: "121.87",
      total: "283.87",
      excessDetails: {
        voluntary: "250",
        compulsory: "350",
        total: "600",
      },
      optionalExtras: [
        { label: "Legal Cover", included: true },
        { label: "Courtesy Car", included: true },
        { label: "Windscreen Cover", included: false },
        { label: "Breakdown Included", included: true },
        { label: "Personal Injury", included: false },
      ],
      buttonText: "View Quote",
    },
    {
      id: 2,
      policyType: "ANNUAL",
      title: "Annual Premium",
      description: "Standard annual insurance with fixed premium and comprehensive coverage.",
      policyholderName: "Sarah Johnson",
      vehicleReg: "SM21 UOL",
      vehicleMake: "Volkswagen",
      vehicleModel: "Golf",
      createdDate: "18 Jan 2024",
      expiresInDays: 25,
      pricePerMonth: "28.99",
      times: "11x",
      deposit: "0",
      total: "347.88",
      excessDetails: {
        voluntary: "250",
        compulsory: "350",
        total: "600",
      },
      optionalExtras: [
        { label: "Legal Cover", included: true },
        { label: "Courtesy Car", included: true },
        { label: "Windscreen Cover", included: true },
        { label: "Breakdown Included", included: false },
        { label: "Personal Injury", included: true },
      ],
      buttonText: "View Quote",
    },
    {
      id: 3,
      policyType: "TEMPORARY",
      title: "Temporary Cover",
      description: "Short-term insurance perfect for test drives, borrowing cars or one-off journeys.",
      policyholderName: "Michael Chen",
      vehicleReg: "BD19 FGH",
      vehicleMake: "BMW",
      vehicleModel: "3 Series",
      createdDate: "20 Jan 2024",
      expiresInDays: 22,
      pricePerMonth: "45.00",
      times: "1x",
      deposit: "0",
      total: "45.00",
      excessDetails: {
        voluntary: "500",
        compulsory: "400",
        total: "900",
      },
      optionalExtras: [
        { label: "Legal Cover", included: false },
        { label: "Courtesy Car", included: false },
        { label: "Windscreen Cover", included: true },
        { label: "Breakdown Included", included: true },
        { label: "Personal Injury", included: true },
      ],
      buttonText: "View Quote",
    },
    {
      id: 4,
      policyType: "ANNUAL",
      title: "Annual Premium",
      description: "Comprehensive annual cover with no mileage restrictions and 24/7 roadside assistance.",
      policyholderName: "Emma Williams",
      vehicleReg: "KP22 XYZ",
      vehicleMake: "Audi",
      vehicleModel: "A4",
      createdDate: "22 Jan 2024",
      expiresInDays: 20,
      pricePerMonth: "38.50",
      times: "11x",
      deposit: "115.50",
      total: "539.00",
      excessDetails: {
        voluntary: "300",
        compulsory: "350",
        total: "650",
      },
      optionalExtras: [
        { label: "Legal Cover", included: true },
        { label: "Courtesy Car", included: true },
        { label: "Windscreen Cover", included: true },
        { label: "Breakdown Included", included: true },
        { label: "Personal Injury", included: true },
      ],
      buttonText: "View Quote",
    },
    {
      id: 5,
      policyType: "PAY PER MILE",
      title: "Pay Per Mile",
      description: "Flexible pay-as-you-drive insurance. Only pay for miles you actually drive.",
      policyholderName: "David Brown",
      vehicleReg: "MX18 ABC",
      vehicleMake: "Toyota",
      vehicleModel: "Corolla",
      createdDate: "23 Jan 2024",
      expiresInDays: 19,
      pricePerMonth: "16.75",
      times: "11x",
      deposit: "83.75",
      total: "268.00",
      excessDetails: {
        voluntary: "250",
        compulsory: "300",
        total: "550",
      },
      optionalExtras: [
        { label: "Legal Cover", included: true },
        { label: "Courtesy Car", included: false },
        { label: "Windscreen Cover", included: true },
        { label: "Breakdown Included", included: true },
        { label: "Personal Injury", included: false },
      ],
      buttonText: "View Quote",
    },
  ];

  return (
    <div className={styles.container}>
      {/* Hero Section */}
      <section className={styles.heroSection}>
        <div className={styles.heroBackground}>
          <svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="106.238px" height="176.262px" viewBox="0 0 106.238 140.262" className={styles.heroBackgroundImage}>
            <style>{`.st0{fill:#FFFFFF;}.st1{fill:#05AFFF;}.st2{fill:#0A0913;}`}</style>
            <path className="st1" d="M86.515,75.233L44.398,94.136v25.204l-4.194-4.194l-13.187-13.187l-7.276-7.276l24.658-11.08l17.44-7.823l35.953-16.152c9.13-4.116,11.334-16.094,4.253-23.175L70.012,4.419C60.57,-5.023,44.398,1.669,44.398,15.012v20.171L0.115,55.081v19.098l17.44-7.823l39.484-17.713l4.916-2.204V27.302l-0.117,0.058v-6.457l24.677,24.677l-10.944,4.916l0.02,0.039l-46.682,21.01l-0.039-0.078L8.464,80.636c-0.351,0.156-0.683,0.312-0.995,0.488c-4.253,2.302-6.808,6.399-7.354,10.768c-0.527,4.175,0.741,8.583,4.077,11.919l32.051,32.032c9.442,9.442,25.594,2.751,25.594-10.612v-19.82l24.677-11.08l19.722-8.837V66.357L86.515,75.233z"/>
          </svg>
        </div>
        <div className={styles.heroContent}>
          <div className={styles.greetingArea} suppressHydrationWarning>
            <h1 className={`${styles.greetingTitle} ${plusJakartaSans?.className || ""}`}>
              Your Quotes
            </h1>
            <p className={styles.greetingSubtitle}>
              Review and compare your insurance quotes
            </p>
          </div>
        </div>
      </section>

      {/* Breadcrumb Navigation */}
      <Breadcrumb items={[
        { label: "Dashboard" },
        { label: "Quotes" }
      ]} />

      {/* Content Wrapper */}
      <div className={styles.contentWrapper}>
        {/* Header with Description */}
        <div className={styles.contentHeader}>
          <div className={styles.contentHeaderText}>
            <h2 className={styles.contentTitle}>Your Quotes</h2>
            <p className={styles.contentSubtitle}>Review your available insurance quotes and choose the best option for you</p>
          </div>
        </div>

        {/* Quotes Grid */}
        <div className={styles.quotesGrid}>
          {quotes.map((quote) => (
            <QuoteCard key={quote.id} quote={quote} />
          ))}
        </div>
      </div>

      <Suspense fallback={<div style={{ padding: "20px", textAlign: "center", color: "#999" }}>Loading...</div>}>
        <NeedHelpSection />
      </Suspense>
    </div>
  );
};

export default QuotesPageClient;
