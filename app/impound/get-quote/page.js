...
const ImpoundInsuranceContent = () => {
  ...
  useEffect(() => {
    ...
    Promise.resolve().then(() => {
      const savedState = sessionStorage.getItem("impoundQuoteFormState");
      if (savedState) {
        try {
  ...
  useEffect(() => {
    ...
    sessionStorage.setItem("impoundQuoteFormState", JSON.stringify(formState));
  ...
  useEffect(() => {
    ...
    const handlePopState = (event) => {
      if (event.state && event.state.type === "impoundQuoteStep") {
        setCurrentStep(event.state.currentStep || STEPS.VEHICLE);
        setVehicleSubStep(event.state.vehicleSubStep || "registration");
        setPersonalSubStep(event.state.personalSubStep || "aboutYou");
        setCoverSubStep(event.state.coverSubStep || "details");
...
const ImpoundInsurancePage = () => {
  return <ImpoundInsuranceContent />;
};
