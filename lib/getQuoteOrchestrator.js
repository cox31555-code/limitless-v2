...
export function useGetQuoteOrchestrator(config) {
  ...
  const handleNextStep = useCallback(async () => {
    const subStep = currentStep === STEP_ENUM.VEHICLE ? vehicleSubStep :
                    currentStep === STEP_ENUM.PERSONAL ? personalSubStep :
                    currentStep === STEP_ENUM.COVER ? coverSubStep : undefined;
    ...
    // Personal sub-step forward navigation
    if (currentStep === STEP_ENUM.PERSONAL) {
      // Skip nested substeps (addClaim, addConviction) in main navigation
      ...
      const currentPersonalIndex = personalSubSteps.indexOf(personalSubStep);

      // Move to next personal substep if available
      if (currentPersonalIndex >= 0 && currentPersonalIndex < personalSubSteps.length - 1) {
        const nextSubStep = personalSubSteps[currentPersonalIndex + 1];
        setPersonalSubStep(nextSubStep);
        pushHistoryState(STEP_ENUM.PERSONAL, vehicleSubStep, nextSubStep, coverSubStep);
        window.scrollTo({ top: 0, behavior: 'smooth' });
        return;
      }

      // Move to Cover after last personal substep (claims)
      if (personalSubStep === 'claims') {
        setCurrentStep(STEP_ENUM.COVER);
        setCoverSubStep(DEFAULT_COVER_SUBSTEP[insuranceType] || 'additionalDrivers');
        pushHistoryState(STEP_ENUM.COVER, vehicleSubStep, personalSubStep, DEFAULT_COVER_SUBSTEP[insuranceType] || 'additionalDrivers');
        window.scrollTo({ top: 0, behavior: 'smooth' });
        return;
      }
    }

    // Cover sub-step forward navigation
    if (currentStep === STEP_ENUM.COVER) {
      // Skip nested substeps in main navigation (addDriver, carOwnerAdd*)
      if (coverSubStep.startsWith('addDriver') || coverSubStep.startsWith('carOwnerAdd')) {
        return;
      }

      const coverSubSteps = ['additionalDrivers', 'carOwner', 'details', 'ncd', 'additionalProducts', 'contactInformation'];
      const currentCoverIndex = coverSubSteps.indexOf(coverSubStep);

      // Move to next cover substep if available
      if (currentCoverIndex >= 0 && currentCoverIndex < coverSubSteps.length - 1) {
        const nextSubStep = coverSubSteps[currentCoverIndex + 1];
        setCoverSubStep(nextSubStep);
        pushHistoryState(STEP_ENUM.COVER, vehicleSubStep, personalSubStep, nextSubStep);
        window.scrollTo({ top: 0, behavior: 'smooth' });
        return;
      }

      // Move to Check Answers after contactInformation
      if (coverSubStep === 'contactInformation') {
        setCurrentStep(STEP_ENUM.CHECK_ANSWERS);
        pushHistoryState(STEP_ENUM.CHECK_ANSWERS, vehicleSubStep, personalSubStep, coverSubStep);
        window.scrollTo({ top: 0, behavior: 'smooth' });
        return;
  ...
  const handlePreviousStep = useCallback(() => {
    // Cover sub-step back navigation - check this BEFORE going back to PERSONAL
    const coverSubSteps = ['additionalDrivers', 'carOwner', 'details', 'ncd', 'additionalProducts', 'contactInformation'];
    const currentCoverIndex = coverSubSteps.indexOf(coverSubStep);

    // If we're on COVER step but not on first substep, navigate within COVER substeps
    if (currentStep === STEP_ENUM.COVER && currentCoverIndex > 0) {
      const prevSubStep = coverSubSteps[currentCoverIndex - 1];
      setCoverSubStep(prevSubStep);
      pushHistoryState(STEP_ENUM.COVER, vehicleSubStep, personalSubStep, prevSubStep);
      window.scrollTo({ top: 0, behavior: 'smooth' });
      return;
    }

    // Only go back to PERSONAL if we're on the first COVER substep
    if (currentStep === STEP_ENUM.COVER && currentCoverIndex === 0) {
      setCurrentStep(STEP_ENUM.PERSONAL);
      setPersonalSubStep('claims');
      pushHistoryState(STEP_ENUM.PERSONAL, vehicleSubStep, 'claims', coverSubStep);
      window.scrollTo({ top: 0, behavior: 'smooth' });
      return;
    }
    ...
    // Personal sub-step back navigation
    const personalSubSteps = ['aboutYou', 'household', 'employment', 'licence', 'restrictions', 'claims'];
    const currentPersonalIndex = personalSubSteps.indexOf(personalSubStep);
    if (currentStep === STEP_ENUM.PERSONAL && currentPersonalIndex > 0) {
      const prevSubStep = personalSubSteps[currentPersonalIndex - 1];
      setPersonalSubStep(prevSubStep);
      pushHistoryState(STEP_ENUM.PERSONAL, vehicleSubStep, prevSubStep, coverSubStep);
      window.scrollTo({ top: 0, behavior: 'smooth' });
      return;
    }
    ...
    // Cover sub-step back navigation already handled above
