...
const ComponentWrapper = ({ children, title, icon, isPaymentPage = false, hideIcon = false }) => {
  const iconSrc = getIconForTitle(title);
  const isVehicleIcon = !hideIcon && title?.toLowerCase().includes("vehicle");
